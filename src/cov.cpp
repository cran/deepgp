  ...
  for (int i = 0; i < n1; i++) {
    for (int j = 0; j < n2; j++) {
      ...
      if (grad1(i) == 0 and grad2(j) == 0) { // K_00
        ...
        d_index = grad1(i) - 1; // change indexing to start at zero
        covmat(i, j) = tau2*K_00*(2/theta)*(1 - (2/theta)*pow(x1(i, d_index) - x2(j, d_index), 2));
      } else { // K_df
        d_index = grad1(i) - 1; // change indexing to start at zero
        f_index = grad2(j) - 1; // change indexing to start at zero
        covmat(i, j) = tau2*K_00*(-4/pow(theta, 2))*(x1(i, d_index) - x2(j, d_index))*(x1(i, f_index) - x2(j, f_index));
      }
    }
  }
  ...
  for (int i = 0; i < n1; i++) {
    for (int j = 0; j < n2; j++) {
      ...
      if (grad1(i) == 0 and grad2(j) == 0) { // K_00
        ...
        d_index = grad1(i) - 1; // change indexing to start at zero
        covmat(i, j) = tau2*K_00*(2/theta(d_index))*(1 - (2/theta(d_index))*pow(x1(i, d_index) - x2(j, d_index), 2));
      } else { // K_df
        d_index = grad1(i) - 1; // change indexing to start at zero
        f_index = grad2(j) - 1; // change indexing to start at zero
  ...
  for (int i = 0; i < n1; i++) {
    for (int j = 0; j < n2; j++) {
      ...
      if (grad1(i) == 0 and grad2(j) == 0) { // K_00
        ...
        d_index = grad2(j) - 1; // change indexing to start at zero
        covmat(i, j) = tau2*5/(3*theta)*(x1(i, d_index) - x2(j, d_index))*(1 + r)*exp(-r);
      } else if (grad1(i) != 0 and grad2(j) == 0) { // K_d0
        d_index = grad1(i) - 1; // change indexing to start at zero
        covmat(i, j) = -tau2*5/(3*theta)*(x1(i, d_index) - x2(j, d_index))*(1 + r)*exp(-r);
      } else if (grad1(i) == grad2(j)) { // K_dd
        d_index = grad1(i) - 1; // change indexing to start at zero
        covmat(i, j) = tau2*5/(3*theta)*exp(-r)*(1 + r - 5/theta*pow(x1(i, d_index) - x2(j, d_index), 2));
      } else { // K_df
        d_index = grad1(i) - 1; // change indexing to start at zero
        f_index = grad2(j) - 1; // change indexing to start at zero
        covmat(i, j) = -tau2*25/(3*pow(theta, 2))*(x1(i, d_index) - x2(j, d_index))*(x1(i, f_index) - x2(j, f_index))*exp(-r);
      }
    }
  }
  // Add nugget to all diagonal elements (acts as jitter for numerical stability)
  ...
  if (v == 0.5) { 
    ...
    for (int i = 0; i < n1; i++) {
      for (int j = 0; j < n2; j++) {
        r = 0.0;
        for (int k = 0; k < d; k++)
          r += 3*pow(x1(i, k) - x2(j, k), 2)/theta(k);
    ...
    for (int i = 0; i < n1; i++) {
      for (int j = 0; j < n2; j++) {
        r = 0.0;
        for (int k = 0; k < d; k++)
          r += 5*pow(x1(i, k) - x2(j, k), 2)/theta(k);
  ...
  for (int i = 0; i < n1; i++) {
    for (int j = 0; j < n2; j++) {
      ...
      if (grad1(i) == 0 and grad2(j) == 0) { // K_00
        ...
        d_index = grad2(j) - 1; // change indexing to start at zero
        covmat(i, j) = tau2*5/(3*theta(d_index))*(x1(i, d_index) - x2(j, d_index))*(1 + r)*exp(-r);
      } else if (grad1(i) != 0 and grad2(j) == 0) { // K_d0
        d_index = grad1(i) - 1; // change indexing to start at zero
        covmat(i, j) = -tau2*5/(3*theta(d_index))*(x1(i, d_index) - x2(j, d_index))*(1 + r)*exp(-r);
      } else if (grad1(i) == grad2(j)) { // K_dd
        d_index = grad1(i) - 1; // change indexing to start at zero
        covmat(i, j) = tau2*5/(3*theta(d_index))*exp(-r)*(1 + r - 5/theta(d_index)*pow(x1(i, d_index) - x2(j, d_index), 2));
      } else { // K_df
        d_index = grad1(i) - 1; // change indexing to start at zero
        f_index = grad2(j) - 1; // change indexing to start at zero
        covmat(i, j) = -tau2*25/(3*theta(d_index)*theta(f_index))*(x1(i, d_index) - x2(j, d_index))*(x1(i, f_index) - x2(j, f_index))*exp(-r);
      }
    }
  }
  // Add nugget to all diagonal elements (acts as jitter for numerical stability)
